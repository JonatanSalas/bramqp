...
const FrameSerializer = module.exports = function FrameSerializer(spec) {
	const self = this;
	this.specData = spec;
...
FrameSerializer.prototype.serializeBitPack = function(buffer, bit, previousBitCount) {
	let localIndex = previousBitCount % 8;
	if (localIndex === 0) {
		this.serializeOctet(buffer, 0);
	}
...
FrameSerializer.prototype.serializeArray = function(buffer, array) {
	if (array instanceof Array) {
		let lengthPosition = buffer.used;
		this.serializeLong(buffer, 0);
		let start = buffer.used;
		for (let i in array) {
...
FrameSerializer.prototype.serializeFrameMethod = function(buffer, channel, className, methodName, fieldData) {
	let methodPayload = new Buffer(this.frameMax);
	methodPayload.used = 0;
	let theClass = this.classLookup[className];
	let method = this.classLookup[className][methodName];
...
FrameSerializer.prototype.serializeFrameContentHeader = function(buffer, channel, className, bodySize, fieldData) {
	let headerPayload = new Buffer(this.frameMax);
	...
	for (let propertiesPresentIndex in propertiesPresent) {
		let localIndex = propertiesPresentIndex % 15;
		let propertyIndex = (propertiesPresentIndex - localIndex) / 15;
		if (localIndex === 0) {
